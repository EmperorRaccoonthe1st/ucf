...
int contains_word(char words[MAX_WORDS][MAX_LEN], int size, char *query) {
    ...
    // TODO 5 BEGIN
    for (int i = 0; i < size; i++) {
        if (strcmp(words[i], query) == 0) return 1;
    }
...
void combine_lists(char list1[MAX_WORDS][MAX_LEN], int list1_size,
    ...
    for (int i = 0; i < list1_size; i++) {
        for (int j = 0; j < list2_size; j++) {
            if (strcmp(list1[i], list2[j]) == 0 && contains_word(list3, *list3_size, list1[i]) == 0) {
...
void selection_sort(char words[MAX_WORDS][MAX_LEN], int size) {
    ...
    for (int i = 0; i < size-1; i++) {
        ...
        for (int j = i+1; j < size; j++) {
            if (strcmp(words[j], words[lPos]) < 0) {
                lPos = j;
            }
        }

        strcpy(tmp, words[i]);
        strcpy(words[i], words[lPos]);
        strcpy(words[lPos], tmp);
    }
