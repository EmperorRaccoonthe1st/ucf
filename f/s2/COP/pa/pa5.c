...
void dwalk(BSTNode *root, int depth);
...
char **filterByTrait(BSTNode *root, int traitIndex, int traitValue, int *resultSize) {
    ...
    *resultSize = amt;
    ...
    return realloc(names, amt * sizeof(char *));
...
void dwalk(BSTNode *root, int depth) {
    ...
    if (root->left != NULL) {
        dprintf(" / ");
        dwalk(root->left, depth + 1);
    } 

    if (root->right != NULL) {
        dprintf(" \\ ");
        dwalk(root->right, depth + 1);
    } 
...
void query4(FILE *ifile, BSTNode *root) {
    ...
    int traitIndex;
    fscanf(ifile, "%d", &traitIndex);

    int traitValue;
    fscanf(ifile, "%d", &traitValue);
