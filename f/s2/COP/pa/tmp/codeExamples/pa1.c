...
char **readBreeds(FILE *ifile, int *count) {
    ...
    for (int i = 0; i < tmpC; i++) {
        ...
        char *breed = malloc((strlen(buff) + 1) * sizeof(char)); 
        strcpy(breed, buff);
        breeds[i] = breed;
    }
...
Kennel *createKennels(FILE *ifile, int kennelCount, char **dictionary, int breedCount, int **constraints) {
    ...
    for (int i = 0; i < kennelCount; i++) {
        ...
        char *location = malloc((strlen(buff) + 1) * sizeof(char));
...
void removeCatFromKennel(Kennel *k, Cat *cat) {
    ...
    Cat **cats = malloc(((k->occupancy) - 1) * sizeof(Cat *));
    for (int i = 0; i < k->occupancy-1; i++) {
        cats[i] = k->cats[i];
        if (k->cats[i] == cat) {
...
void freeStore(CatStore *store) {
    ...
    for (int i = 0; i < store->numKennels; i++) {
        for (int x = 0; x < store->kennels[i].occupancy; x++) {
            free(store->kennels[i].cats[x]->name);
            free(store->kennels[i].cats[x]);
        }
...
void query1(FILE *ifile, CatStore *s, char **dictionary, int breedCount) {
    ...
    for (int i = 0; i < s->numKennels; i++) {
        breedAmt += findBreedAmt(dictionary, breed, &(s->kennels[i]));
    ...
    if (canMoveTo(s, location, cat->breed, dictionary, breedCount) == 1) {
         for (int i = 0; i < s->numKennels; i++) {
            if (strcmp(s->kennels[i].location, location) == 0) {
                ...
                s->kennels[i].occupancy += 1;

                Cat **cats = malloc(s->kennels[i].occupancy * sizeof(Cat *));
                for (int x = 0; x < s->kennels[i].occupancy - 1; x++) {
                   cats[x] = s->kennels[i].cats[x]; 
                }
                cats[s->kennels[i].occupancy - 1] = cat;
                free(s->kennels[i].cats);

                s->kennels[i].cats = cats;
            }
        }         
